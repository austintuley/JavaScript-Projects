...
function placeXOrO(squareNumber) {
	...
	if (!selectedSquares.some((element) => element.includes(squareNumber))) {
		...
		if (activePlayer === 'O') {
			...
			disableClick();
			// This funtion waits 1 second before placing the image
			// and enabling click.
			setTimeout(function() {
				computersTurn();
			}, 1000);
...
function checkWinConditions() {
	...
	// This function checks if an array inclues 3 strings.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
	...
	function animateLineDrawing() {
		...
		c.stroke();
		// This conditioin checks if we've reached the endpoint.
		if (x1 <= x2 && y1 <= y2) {
